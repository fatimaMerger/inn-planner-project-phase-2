...
export class AdminPrivacyPolicyComponent implements OnInit {
 ...
 validateForm() {
   if (!this.   updatePrivacyForm.valid) {
 ...
 handleFormError() {
   for (const i in this.  updatePrivacyForm.controls) {
     if (i) {
       this.  updatePrivacyForm.controls[i].markAsDirty();
       this.  updatePrivacyForm.controls[i].markAsTouched();
       this.  updatePrivacyForm.controls[i].setErrors({ newerror: true });
       this.  updatePrivacyForm.controls[i].updateValueAndValidity();
     }
   }
 }
 save() {
